...
export class MetricService {
  ...
  getObservableCounter(name: string, options?: MetricOptions, callback?: (observableResult: ObservableResult) => void) {
    return getOrCreateObservableCounter(name, options, callback);
  }

  getObservableGauge(name: string, options?: MetricOptions, callback?: (observableResult: ObservableResult) => void) {
    return getOrCreateObservableGauge(name, options, callback);
  }

  getObservableUpDownCounter(name: string, options?: MetricOptions, callback?: (observableResult: ObservableResult) => void) {
